...
**Trend Sparkline**:
The Lighthouse Widget's faint per-cell line tracing one metric's raw value across the retained Lighthouse Snapshot history, newest point at the right — shown for Performance, LCP, CLS, and TBT. Plotted against a fixed threshold-anchored vertical scale so lab noise stays visually small, and deliberately secondary to the trend arrow: the arrow, not the line, is the authoritative tier-change signal and the assistive-tech trend story ([ADR 0009](docs/adr/0009-trend-sparkline-raw-history.md)). Build-time inline SVG, decorative (`aria-hidden`), omitted when fewer than two runs exist.
