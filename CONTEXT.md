...
**Install Button**:
The header control (before the Theme Toggle, duplicated across the desktop and mobile nav blocks) that fires the browser's install prompt for the PWA. Chromium-only: hidden where the browser has no programmatic install (Safari/iOS, Firefox) and when the site is already running installed. Deliberately not an iOS "Add to Home Screen" instruction UI — that path stays on the browser's own Share menu (guntherjh/guntherjh.github.io#146).
