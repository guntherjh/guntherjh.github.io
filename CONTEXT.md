...
**Lighthouse Widget**:
The About page's "This site" subsection's display of the latest Lighthouse Snapshot — category scores (Performance, Accessibility, Best Practices, SEO) and lab Core Web Vitals proxies (LCP, CLS, TBT) for Home, About, Resume, and the Blog index. A compact grid, not prose/cards, matching the site's data-dense visual direction (guntherjh/guntherjh.github.io#41); it reflows to one stacked card per page below a phone-width breakpoint so it never scrolls sideways (guntherjh/guntherjh.github.io#124). Each score/metric cell is color-coded against Lighthouse's/Core Web Vitals' own good/needs-improvement/poor thresholds, and shows a trend indicator against the immediately-previous run in the Snapshot's history (guntherjh/guntherjh.github.io#71, [ADR 0006](docs/adr/0006-lighthouse-snapshot-history.md)). A collapsible definition list under the table explains each metric column and its thresholds (each header wired to its definition via `aria-describedby`); trend arrows (↑/↓) that mark an actual tier change are toggletips (native HTML Popover, no JS) revealing the previous run's value (guntherjh/guntherjh.github.io#124, [ADR 0007](docs/adr/0007-trend-toggletip-native-popover.md), sourced in [docs/research/lighthouse-metric-definitions.md](docs/research/lighthouse-metric-definitions.md)).
