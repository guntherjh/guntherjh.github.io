...
- **Linting and formatting**: ESLint (`eslint:recommended`, flat config in `eslint.config.js`) and Prettier (zero-config defaults), both scoped to `.js` files only — not repo-wide, since Prettier doesn't safely handle Nunjucks `.njk` templates. `npm run lint` / `npm run format` / `npm run format:check`. Enforced as required checks on every PR (see Git workflow below) — not just a local nicety.
