...
- **Required checks**: every PR against `master` must pass three checks — lint, format-check, build (`.github/workflows/checks.yml`) — enforced via branch protection on `master`. These automate what the two bullets above ask you to do manually; CI is the backstop, not a substitute for checking locally first.
