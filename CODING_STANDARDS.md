...
- **Theme toggle** (`src/js/theme-toggle.js`): binary light/dark, defaults to `prefers-color-scheme` until the visitor explicitly overrides it, sticky `localStorage` override. The *initial* theme is applied by a deliberately inline (not external/deferred) `<script>` in `base.njk`'s `<head>`, before first paint — an external script would load after the initial render and cause a visible flash of the wrong theme. `theme-toggle.js` only owns the interactive part (click handler, persistence, following system-preference changes while no override is stored). Same `localStorage` pattern as `src/js/about-sections.js`.
