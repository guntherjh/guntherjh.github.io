...
- The panel is bottom-centred (`position: fixed`), not tethered to the arrow — CSS anchor positioning isn't broadly supported yet, and the top-layer panel would otherwise land at the viewport origin. The panel leads with its own metric and value ("Performance 100 (good)…") so it's unambiguous which arrow it belongs to. **Revised by [ADR 0008](0008-metric-definition-toggletips.md):** where anchor positioning is available the panel now sits against its ↑/↓ button; the bottom-centred rule is the fallback, which is why the self-identifying lead-in stays.
